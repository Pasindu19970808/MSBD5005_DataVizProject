...
def collectParticipantTimeStampBoundaries(df,id_alias,id,complete_df):
    ...
    result_dict = collectParticipantTimeStampPoints(id_df,complete_df,id,'time_complete',id_alias)
    return result_dict
...
def collectTimeStampLocationPoints(df,id_alias,complete_df):
    ...
    for i,id in enumerate(df[COL_MAP[id_alias]].unique()):
        ...
        result_dict = collectParticipantTimeStampBoundaries(df,id_alias,id,complete_df)
        # for j,linestring in iter_gdf:
        #     linestring_dict[f"{id}_{j}"] = linestring
        try:
            linestring_dict[id] = {j:(linestring,time_diff) for j,linestring,time_diff in result_dict}
        except:
