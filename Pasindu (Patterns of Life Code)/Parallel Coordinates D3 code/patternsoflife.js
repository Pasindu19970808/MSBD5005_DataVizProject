...
parallelCoords = [{filename:'parallelCoordsExpense_individual.csv',svgnum:1}]
...
let plotClusterFunc = (result,svgnum) => {
    ...
    const height = 300
    const margin = {
    left: 60, top: 15, right: 10, bottom: 20
    }
    ...
    const scaleLegend = d3.scaleOrdinal()
...
async function plotParallelCoords(data,svgnum){
    ...
    const scaleLegend = d3.scaleOrdinal()
    // .domain(['Cluster 1','Cluster 2','Cluster 3'])
    .domain(['Participant 28','Participant 855','Participant 235'])
    .range(['green','red','orange'])

    //https://d3-legend.susielu.com/
    const legend = d3
    .legendColor()
    .shape("line", d3.symbol().type(d3.symbolCircle).size(25)())
    .shapePadding(15)
    .labelOffset(5)
    .scale(scaleLegend)
    .labels(['Participant 28','Participant 855','Participant 235'])

    svgparallel.append("g")
    .attr("class", "legend_auto")
    .style('font-size', 20)
    .style('font-family', 'sans-serif')
    .attr("transform", `translate(${width - margin.right + 5}, ${margin.top + 40})`)
    .call(legend)

    d3.selectAll(`#parallelCoords${svgnum} .tick`)
    .attr('font-size','1rem')
    .attr('font-weight','bold')

    // d3.selectAll(`#parallelCoords${svgnum} .tick .text`)
